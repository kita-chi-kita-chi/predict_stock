...
st.write("※あくまでもAIによる予測です（参考値）。こちらのアプリによる損害や損失は一切補償しかねます。")
...
try:
    ...
    df_stock = yf.download(stock_name, start="2021-01-05")
    ...
    st.header(stock_name + "終値と14日間移動平均（USD)")
    df_stock['SMA'] = df_stock['Close'].rolling(window=14).mean()
    df_stock2 = df_stock[['Close', 'SMA']]
    st.line_chart(df_stock2)

    st.header(stock_name+"値動き")
    df_stock['change']=(((df_stock['Close']-df_stock['Open']-df_stock['Open']))/(df_stock['Open'])*100)
